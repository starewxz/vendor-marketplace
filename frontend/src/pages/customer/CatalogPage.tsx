...
function FacetGroup({
  ...
  return (
    <div className="flex flex-col gap-2">
      <h3 className="text-xs font-semibold tracking-wide text-navy/50 uppercase">{title}</h3>
      <div className="flex flex-col gap-1">
        {Object.entries(facet).map(([key, count]) => (
          <button
            ...
            className={`flex items-center justify-between rounded-lg px-2 py-1 text-left text-sm ${
              active === key ? 'bg-cargo-yellow text-navy' : 'text-navy/70 hover:bg-cream'
            }`}
          >
            <span>{labelFor ? labelFor(key) : key}</span>
            <span className="text-xs text-navy/40">{count}</span>
          </button>
        ))}
      </div>
    </div>
...
export function CatalogPage() {
  ...
  const { data, isLoading, isError } = useCatalog(query);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        {data && <p className="text-sm text-navy/60">{data.meta.total} product(s)</p>}
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-[220px_1fr]">
        <aside className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <h3 className="text-xs font-semibold tracking-wide text-navy/50 uppercase">Price</h3>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-end">
            <select
              ...
              className="rounded-lg border border-line px-3 py-1.5 text-sm text-navy"
              aria-label="Sort"
            >
