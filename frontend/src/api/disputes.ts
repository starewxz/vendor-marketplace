...
export const resolveDispute = async (id: string, input: { outcome: 'CUSTOMER' | 'SELLER'; adminResolution: string; refund?: { sellerOrderItemId: string; quantity: number; reason?: string } }, idempotencyKey: string) => (await apiClient.post<Dispute>(`/admin/disputes/${id}/resolve`, input, { headers: { 'Idempotency-Key': idempotencyKey } })).data;
